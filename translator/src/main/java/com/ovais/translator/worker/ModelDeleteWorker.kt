...
@HiltWorker
class ModelDeleteWorker @AssistedInject constructor(
    ...
    companion object {
        private const val TAG = "tag:ModelDeleteWorker"
        private const val MODEL_NAME = "translation_model"
        fun buildWorker(
            model: String
        ) = OneTimeWorkRequestBuilder<ModelDeleteWorker>()
        ...
        return inputData.getString(MODEL_NAME)?.let { model ->
            try {
                withContext(limitedCoroutineContext) {
                    when (val result = translationModelManager.delete(model)) {
    ...
    private fun createErrorNotification(error: String) {
        notificationManager.notify("Model Deletion Failed", error)
    }
}